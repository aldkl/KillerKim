...
void CSetPlayer::CreatePopupWindow(SAMPLE_HANDLE ClickSnd, int nLayer, CRootScene* pScene, bool bShow)
{
	Reset();
	...
	m_Btns[0].pUI[1]->SetTexture("Data/Image/Imgs/1P1.png");
	m_Btns[0].pUI[1]->SetPos(stEMVec3(314, 250, 0));
	m_Btns[0].pUI[1]->SetSize(0.4f);
	...
	pScene->Commit(nLayer, "D_1PBtn", m_Btns[0].pUI[1]);

	//===============================================================
	//2P 버튼
	m_Btns[1].pUI[0] = new CEMPlane();
	m_Btns[1].pUI[0]->SetTexture("Data/Image/Imgs/2P1.png");
	m_Btns[1].pUI[0]->SetPos(stEMVec3(314, 250, 0));
	m_Btns[1].pUI[0]->SetSize(0.4f);
	...
	pScene->Commit(nLayer, "D_2PBtn", m_Btns[1].pUI[0]);
...
void CSetPlayer::OnMouseMoveEvent(stMouseInfo stPos)
{
	if (GetCollisionFlag())
	{
...
void CSetPlayer::OnMouseUpEvent(stMouseInfo stPos, enumMouseButton eButton)
{
	if (GetCollisionFlag())
	{
		switch (eButton)
		...
		case E_MOUSE_LEFT:
		{
			...
			if (m_pSelectPlane)
			{
				...
				if (m_pSelectPlane->GetName() == "D Sound_ON")
				{
					D_SOUND->Play(m_ClickSnd, D_MNG->GetSoundVolume());
