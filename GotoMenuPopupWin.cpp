...
void CGotoMenuPopupWin::CreatePopupWindow(SAMPLE_HANDLE ClickSnd, int nLayer, CRootScene* pScene, bool bShow)
{
	Reset();
	...
	pScene->Commit(nLayer, "A_GotoMenuPopup", m_pBg);

	//===============================================================
	// 팝업 Title
	m_pEtcs[0] = new CEMPlane();
	m_pEtcs[0]->SetTexture("Data/Image/Imgs/gotomenu.png");
	...
	pScene->Commit(nLayer, "B_GotoMenu_Title", m_pEtcs[0]);
...
void CGotoMenuPopupWin::OnMouseMoveEvent(stMouseInfo stPos)
{
	if (GetCollisionFlag())
	{
...
void CGotoMenuPopupWin::OnMouseUpEvent(stMouseInfo stPos, enumMouseButton eButton)
{
	if (GetCollisionFlag())
	{
		switch (eButton)
		...
		case E_MOUSE_LEFT:
		{
			...
			if (m_pSelectPlane)
			{
				...
				if (m_pSelectPlane->GetName() == "D_CancelBtn")
				{
					...
					//GotoMenuPopupWin를 안보이게한다.
					SetShow(false);

					//GameScene 안에 GameScene_BaseUI 충돌처리를 가능하게한다.
					//CGameScene* pGameScene = (CGameScene*)D_SCENE->GetCurScene();
					//pGameScene->m_BaseUI.SetCollisionFlag(true);
				}
			}
		}break;
